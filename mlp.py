...
class MLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, activation=T.tanh):
        ...
        self.hiddenLayers.append(firstLayer)

        for i in range(1,len(n_hidden)):
            # take last hidden layer's output as input
            # take last 
            tempLayer = HiddenLayer(
                rng=rng,
            ...
            self.hiddenLayers.append(tempLayer)
            print "add hidden layer ", i , " with unit", n_hidden[i] 
...
def printParameters(classifier):
...
def test_mlp(learning_rate=0.01, L1_reg=0.001, L2_reg=0.0001, n_epochs=1000,
             dataset='data', batch_size=20, n_hidden=[500], activation=T.tanh):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--data", type=str, default='data', help="data file to read in")
    args = parser.parse_args()

    rectifier = lambda x: T.maximum(0, x)
    test_mlp(dataset=args.data, n_hidden = [40, 40])
