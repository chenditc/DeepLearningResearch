...
#    cursor.execute('SELECT * FROM data WHERE Ticker=%s', (security) )
    ...
    for i in range(len(dataLines)- days - 1):
        ...
        # get y value
        day_1 = dataLines[i + days]
        day_2 = dataLines[i + days + 1]
        day_1_price = float(day_1[2]) # get open price
        day_2_price = float(day_2[2]) # get open price
        percentChange = (day_2_price - day_1_price) * 100.0 / day_1_price 
