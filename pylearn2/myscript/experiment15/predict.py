...
for dateDelta in range(300):
    ...
    dataset, indexToSecuirty = load_security_test.load_security('all', startDate = startDate, stopDate = stopDate, days = 5, threshold = 0)

    x_test = dataset.X

    y = predictFunction( x_test )
    ...
    for i in range(5):
        ...
        realChange =  dataset.y[indexToSecuirty.index(security)]
        pnl = 0
        if predictChange > 0:
            pnl += realChange
        else:
